...
*     II = 1
*     IF (IPN.LE.1) II = 100
*     IF (NSTEP1.GT.200000.AND.IPN.EQ.2) II = 5
*     IF (NSTEP1.GT.500000.AND.IPN.EQ.2) II = 10
*     IF (MOD(NSTEP1,II).EQ.0) CALL BBHNB(SEMI,ECC,IPN)
**************
*     WRITE (6,132)  TNOW, ECC, SEMI
* 132 FORMAT (' ELEMENTS   T E A ',F12.5,F9.5,1P,E12.4)
*
...
*         IF (ECC.GE.0.9D0) II = 2
          ...
          IF (MOD(NSTEP1,II).EQ.0) THEN
              ...
              WRITE (57,141)  IPN, TNOW,
     &              NAMEC(I1), NAMEC(I2), ISTAR(I1), ISTAR(I2),
     &              M(I1), M(I2), ECC, SEMI,
     &              (X(K+KI),K=1,3), (X(K+KN),K=1,3),
     &              (V(K+KI),K=1,3), (V(K+KN),K=1,3),
     &              EB, NPERT, GPERT
  141         FORMAT (' BBH (CHAIN)   IP T NAM K* M E A X V EB NP G ',
     &                       I3,F14.5,
     &                       2I10,2I4,
     &                       1P,2E12.2,0P,F12.7,1P,E12.4,0P,
     &                       12F20.12,
     &                       F12.6,I3,1P,E9.1)
              CALL FLUSH(57)
              CALL BBHNB(SEMI,ECC,IPN)
          END IF
*     END IF
*
...
*       Allow for 2nd order correction (Mikkola & Merritt ApJ 135, 2398).
              ...
              IF (DW.GT.1.0D-03) THEN
                  ...
                  DW = DW*(1.0 + 5.0*DW)
                  WRITE (6,142)  NSTEP1, IPN, IX, ECC, SEMI, DW
  142             FORMAT  (' EINSTEIN SHIFT    # IPN IX E A DW ',
     &                                         I7,2I4,F9.5,1P,2E10.2)
              END IF
          ELSE
              IEI = 0
          END IF
      END IF
*
      IF (IPN.GE.2.AND.MOD(NSTEP1,5).EQ.0) THEN
          CALL CONST(X,V,M,N,ENER0,G0,AL)
          WRITE (6,144)  NSTEP1, IPN, ECC, ENERGY, ENERGR, SEMI,
     &              NAMEC(I1), NAMEC(I2), ISTAR(I1), ISTAR(I2),
     &              M(I1), M(I2),
     &              GPERT, ESUM, (1.0/RINV(K),K=1,N-1)
  144     FORMAT (' WATCH    # IPN E EN EGR A NAM K* M GP ES R ',
     &                       I8,I4,F12.7,2F12.7,1P,E12.4,
     &                       2I10,2I4,1P,2E12.2,7E12.2)
          DW = 3.0*TWOPI*(M(I1) + M(I2))/(SEMI*Clight**2*(1.0 - ECC2))
******************
          write(6,*) 'WATCH* T N IPN DW NAM K M', TNOW, N, IPN, DW,
     &                    (NAMEC(K),K=1,N), (ISTAR(K),K=1,N),
     &                    (M(K),K=1,N)
******************
      END IF
*
...
*       Note small correction from (1 - e1) to (1 - e1**2) 6/2012.
              TKOZ = TOUT**2/TIN*(1.0 - ECC1)**1.5*(1.0 + MB/M(IM))
*       Include numerical factor quoted by Fabrycky & Tremaine 2007 (9/11).
              TKOZ = 2.0/(3.0*3.14)*TKOZ  ! Kiseleva et al MN 300, 292, 1998.
******************
              IF (IPN.GE.0) THEN
                 WRITE (571,211) IPN, TNOW,
     &           NAMEC(I1), NAMEC(I2), NAMEC(IM),
     &           ISTAR(I1), ISTAR(I2), ISTAR(IM),
     &           M(I1), M(I2), M(IM),
     &           ECC, SEMI, SQRT(ECC1), AOUT, TIN, TOUT, TKOZ
  211            FORMAT (I3,F14.5,3I10,3I4,1P,3E12.2,0P,
     &                   F12.7,1P,E12.2,0P,F12.7,1P,E12.2,3E10.2)
                 CALL FLUSH(571)
              END IF
******************
...
******************
              IF (IPN.GE.0) THEN
                 WRITE (572,231) IPN, TNOW,
     &           NAMEC(I1), NAMEC(I2), NAMEC(IM),
     &           ISTAR(I1), ISTAR(I2), ISTAR(IM),
     &           M(I1), M(I2), M(IM),
     &           ECC, SEMI, ECC1, AOUT, TKOZ
  231            FORMAT (I3,F14.5,3I10,3I4,1P,3E12.2,0P,
     &                   F12.7,1P,E12.2,0P,F12.7,1P,E12.2,E10.2)
                 CALL FLUSH(572)
              END IF
******************
          END IF
...
*****************
*****************
              write(6,*) 'COALESCENCE* T N IPN NAM K M', TNOW, N, IPN,
     &                    (NAMEC(K),K=1,N), (ISTAR(K),K=1,N),
     &                    (M(K),K=1,N)
*****************
...
*       Include termination condition for N = 2 or large perturbation.
     ...
     &    (N.GE.3.AND.GPERT.GT.0.1.AND.NSTEP1.GT.NEXT-10).OR.
...
*       Sort inverse distances RINV for correct termination.
          ...
          ELSE IF (ISORT(1).EQ.1) THEN
              IESC = INAME(1)
              JESC = INAME(2)
              IF (1.0/RINV(2).LT.0.01*RSUM) JESC = 0
          ELSE
              IESC = INAME(N-1)
              JESC = INAME(N)
*       Include case of small R2 or large R3 for single removal of INAME(N).
              IF (1.0/RINV(2).LT.0.01*RSUM.OR.
     &            1.0/RINV(3).GT.0.8*RSUM) THEN
                  IESC = INAME(N)
                  JESC = 0
              END IF
          END IF
...
*       Include same treatment for N > 4 except RETURN after reduction.
          CALL HPSORT(N-1,RINV,ISORT)
*       Skip for intermediate large distances (cannot be removed).
          IF (ISORT(1).EQ.2.OR.ISORT(1).EQ.3) THEN
              GO TO 400
          END IF
          WRITE (6,288)  (ISORT(K),K=1,N-1), (INAME(K),K=1,N)
  288     FORMAT (' FIVE-BODY CHAIN TERM   ISORT INM ',10I4)
          IF (ISORT(1).EQ.1) THEN
              IESC = INAME(1)
              JESC = INAME(2)
              IF (1.0/RINV(2).LT.0.1*1.0/RINV(1)) JESC = 0
          ELSE IF (ISORT(1).EQ.N-1) THEN     ! Check last chain member(s).
              IESC = INAME(N-1)
              JESC = INAME(N)
              IF (1.0/RINV(N-2).LT.0.03*RSUM) THEN
                  JESC = 0         ! Note reduction by 1 inside 0.03*RSUM.
                  IESC = INAME(N)  ! Switch to last body.
              END IF
          END IF
          WRITE (6,289)  IESC, JESC, N, 1.0/RINV(IESC)
  289     FORMAT (' FIVE-BODY CHAIN REDUCE    IE JE N R ',3I4,1P,E10.2)

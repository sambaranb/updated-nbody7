...
*       Check optional output (every 100 orbits or each time for #42 > 1).
      ...
      IF ((MOD(IT,100).EQ.0.AND.EMAX.GT.0.99).OR.KZ(42).GT.1) THEN
          WRITE (42,30)  TIME+TOFF, NAME(I1), NAME(I2),
     &    KSTAR(I1), KSTAR(I2), BODY(I1)*ZMBAR, BODY(I2)*ZMBAR,
     &                   ZI, EMIN, EMAX,
     &                   ECC, ECC1, SEMI, SEMI1/SEMI, TKOZ, TMDIS(IM)
   30     FORMAT (F14.5,2I10,2I4,2F9.2,F7.1,F6.2,3F10.6,1P,
     &              E12.2,0P,F6.1,1P,2E9.1)
          CALL FLUSH(42)

...
********
          IF (KZ(43).GE.3) THEN
             ...
             ZM2 = BODY(I2)*ZMBAR
             NMI = NAME(I1)
             IF(ZM1.LT.ZM2) NMI = NAME(I2)
          CALL GWREC3(NAME(I1),NAME(I2),ZM1,ZM2,KSTAR(I1),KSTAR(I2),
     &vrec,afin,sfin,thfin)
             VFIN2 = 0.0D0
             DO 481 K = 1,3
                 XDOT(K,I) = XDOT(K,I) + (vrec(K)/VSTAR)
                 X0DOT(K,I) = XDOT(K,I)
                 VFIN2 = VFIN2 + XDOT(K,I)**2
  481        CONTINUE
             SPIN(I) = sfin/SPNFAC
             SPN(NMI) = SPIN(I)
             ASPN(NMI) = afin
             ECD0 = ECDOT
             ECDOT = ECDOT + 0.5*BODY(I)*(VI20 - VFIN2)
             VESC = SQRT(VFIN2)
             VF = VESC/SQRT(VI20)
             VESC = VESC*VSTAR
          ELSE

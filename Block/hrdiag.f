...
* Reduce the mass to the gravitational mass for the relevant cases. 
                  ...
                  if(psflag.lt.0)then
* Woosley (2017) PPSN/PSN model
* Woosley S. E., 2017, ApJ, 836, 244
* Spera M., Mapelli M., 2017, MNRAS, 470, 4739
* Giacobbo N., Mapelli M., Spera M., 2018, MNRAS, 474, 2959
* (psflag < 0)
                     Ffac = mcbagb/mt
                     Kfac = 0.67000d0*Ffac + 0.10000d0 
* H-rich star (normal star)
                     if(mcbagb.le.32.d0)then
                        fp = 1.0d0
                     elseif(32.d0.lt.mcbagb.and.mcbagb.le.37.d0)then
                        fp = 0.2d0*(Kfac-1.d0)*mcbagb + 
     &                                       0.2d0*(37.d0-32.d0*Kfac)
                     elseif(37.d0.lt.mcbagb.and.mcbagb.le.60.d0)then
                        fp = Kfac
                     elseif(60.d0.lt.mcbagb.and.mcbagb.lt.64.d0)then
                        fp = -0.25d0*Kfac*mcbagb + 16.0d0*Kfac
                     elseif(64.d0.le.mcbagb.and.mcbagb.lt.135.d0)then
                        fp = 0.0d0
                        kw = 15
                     elseif(135.d0.le.mcbagb)then
                        fp = 1.0d0
                     endif
                     mt = fp*mt
                  endif
                  mc = mt
*
                  if(mt.gt.0.d0.and.mt.le.mxns)then
*
* Zero-age Neutron star
*
                     kw = 13
                  elseif(mt.gt.0.d0)then
*
* Zero-age Black hole
*
                     kw = 14
...
* (psflag < 0)
                        Ffac = mass/mt
* He star (WR star)
                        if(mass.le.32.d0)then
                           fp = 1.0d0
                        elseif(32.d0.lt.mass.and.mass.le.37.d0)then
                           fp = (mass-32.0d0)*
     &                          (0.52260d0*Ffac - 0.52974d0) + 1.0d0
                        elseif(37.d0.lt.mass.and.mass.le.56.d0)then
                           fp = (-0.1381d0*Ffac+0.1309d0)*
     &                           (mass-56.0d0) + 0.82916d0
                        elseif(56.d0.lt.mass.and.mass.lt.64.d0)then
                           fp = -0.103645d0*mass + 6.63328d0
                        elseif(64.d0.le.mass.and.mass.lt.135.d0)then
                           fp = 0.0d0
                           kw = 15
                        elseif(135.d0.le.mass)then
                           fp = 1.0d0
                        endif
                        mt = fp*mt
                     endif

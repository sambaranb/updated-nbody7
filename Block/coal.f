...
*       Delay inclusion of any mass loss until after energy correction.
      ...
      NAME1 = NAME(I1)
      NAME2 = NAME(I2)
*****  Enforce mass loss from the secondary or the non-compact member!!
*      (Override old DM and ZMNEW)
*      (c.f. 'mix.f')
      IF (KSTAR(I1).LT.10.AND.KSTAR(I2).LT.10) THEN
         DM = fmrg*ZM2
         IF (ZM2.GT.ZM1) DM = fmrg*ZM1
      ELSEIF (MAX(KSTAR(I1),KSTAR(I2)).GE.13) THEN
         DM = 0.0D0
         IF (KSTAR(I1).LT.10) DM = ZM1
         IF (KSTAR(I2).LT.10) DM = ZM2
         IF (MAX(KSTAR(I1),KSTAR(I2)).EQ.14) DM = (1.0D0 - ftzacc)*DM
      ELSE
         DM = 0.0D0
      END IF
      WRITE(6,*) "Enforced mass loss (COAL): DM = ", DM
      DM = DM/ZMBAR
      ZMNEW = ZM - DM
*****
...
*
     ...
     &               RI/RC, RIJ*SU, ECC, TK
   50 FORMAT (1X,F11.2,2I8,3I6,3F9.2,2F12.3,F9.1,F10.2,F12.7,1P,E12.1)
      CALL FLUSH(12)
*

...
*       Perturber index from routine IMPACT (ICOMP & JCOMP set in KSTERM).
      ...
      NAME3(3) = ICOMP
*
*       Ensure heaviest reference body.
      IF (BODY(JCLOSE).GT.BODY(ICOMP)) THEN
          NAME3(3) = JCLOSE
          NAME3(2) = ICOMP
          NAME3(1) = JCOMP
      END IF
...
*
*       Ensure ICOMP < JCOMP for KS treatment (suppress for consistency).
*     K = ICOMP
*     ICOMP = MIN(ICOMP,JCOMP)
*     JCOMP = MAX(K,JCOMP)
*
*       Select consistent neighbours for single body and new KS components.
      RS0 = RS(ICM)
      CALL NBLIST(I,RS0)
      CALL NBLIST(ICOMP,RS0)
      CALL NBLIST(JCOMP,RS0)    ! Bug fix 9/19.
*

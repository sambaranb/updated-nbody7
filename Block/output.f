...
*
   ...
   40 FORMAT (//,' T =',F7.0,'  N =',I8,'  <NB> =',I3,'  KS =',I6,
     &           '  NM =',I4,'  MM =',I2,'  NS =',I8,
...
*
     ...
     &              TCR, I6, NESC, VRMS
   55 FORMAT (' #1',F5.2,F6.1,F6.2,F8.4,I8,F7.3,F6.0,F7.0,F6.0,F6.1,
...
*       Convert masses, coordinates & velocities to single precision.
      DO 90 I = 1,NTOT
          ...
          DO 85 K = 1,3
              ...
              VS(K,I) = XDOT(K,I)
   85     CONTINUE
*       Register the steller types too.
          kstars(I) = KSTAR(I)
   90 CONTINUE
*
...
*           write(*,*) I, body(I)*SMU, kstar(I), RI
*        END IF
         IF (KSTARS(I).EQ.14) THEN
            bhnum = bhnum + 1
            mbh = body(I)*SMU
            if( (KZ(24).GE.15).OR.(KZ(24).LE.-1) ) then
            write(120,*) 'BH ', TIME, I, NAME(I), mbh, kstars(I),
     &           X(1,I)-RDENS(1), X(2,I)-RDENS(2), X(3,I)-RDENS(3),
     &           XDOT(1,I), XDOT(2,I), XDOT(3,I)
            cout = .TRUE.
            end if
         ELSE IF (KSTARS(I).EQ.13) THEN
            nsnum = nsnum + 1
            mns = body(I)*SMU
            if( (KZ(24).GE.16).OR.(KZ(24).LE.-2) ) then
            write(120,*) 'NS ', TIME, I, NAME(I), mns, kstars(I),
     &           X(1,I)-RDENS(1), X(2,I)-RDENS(2), X(3,I)-RDENS(3),
     &           XDOT(1,I), XDOT(2,I), XDOT(3,I)
            cout = .TRUE.
            end if
         ELSE IF ( ((KSTARS(I).GE.10).AND.(KSTARS(I).LT.13)) ) THEN
            wdnum = wdnum + 1
            mwd = body(I)*SMU
            if( (KZ(24).EQ.17).OR.(KZ(24).EQ.-3) ) then
            write(120,*) 'WD ', TIME, I, NAME(I), mwd, kstars(I),
     &           X(1,I)-RDENS(1), X(2,I)-RDENS(2), X(3,I)-RDENS(3),
     &           XDOT(1,I), XDOT(2,I), XDOT(3,I)
            cout = .TRUE.
            end if
...
*
      ...
      CALL FLUSH(120)
*
      write(*,*) 'OUTPUT: ', TIME, ' NBH = ', bhnum, ' NNS = ', nsnum,
     &                    ' NWD = ', wdnum
*******
*     CLOSE (UNIT=3)
*

...
*
     ...
     &       JSPIN(2),OSPIN(2),DJSPIN(2),DTMI(2)
      ...
      REAL*8 K2,K3,ACC1,AURSUN
      PARAMETER(K3=0.21D0,ACC1=3.920659D+08)
...
*            DMA(K) = MIN(DMA(K),0.8D0*DMR(3-K))
     ...
     &                    PSII*DMA(K)*RADX(3-K)*RADX(3-K)*OSPIN(3-K))*DT
             KW = KSTAR(J)
             IF(KW.LT.10)THEN
...
*
     ...
     &                PSII*DMA(1)*RADX(2)*RADX(2)*OSPIN(2))*DTM0
         DJSPIN(2) = (2.D0/3.D0)*(DMR(2)*RADX(2)*RADX(2)*OSPIN(2) -
     &                PSII*DMA(2)*RADX(1)*RADX(1)*OSPIN(1))*DTM0

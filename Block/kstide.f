      SUBROUTINE KSTIDE(IPAIR,KCASE,QPERI)
*
*
...
*       Skip significant perturbation near pericentre.
      IF (GAMMA(IPAIR).GT.0.0001) GO TO 50
*
...
*       Form current semi-major axis & eccentricity (not at peri).
      ...
      HI = H(IPAIR)
*
*      Switch to KZ(27)=3 mode if both members are compact
      IF (KSTAR(I1).GE.13.AND.KSTAR(I2).GE.13) DUBCMP = .TRUE.
*
*       Distinguish between sequential circularization, standard chaos & GR.
      R1 = MAX(RADIUS(I1),RADIUS(I2))
      IF (KZ(27).EQ.1.AND.(DUBCMP.EQV..FALSE.)) THEN
          ZF = 4.0
...
*       Skip if penetration is not significant (prevents frequent calls).
      ...
      ELSE IF (KZ(27).EQ.2.AND.(DUBCMP.EQV..FALSE.)) THEN
          IS(1) = KSTAR(I1)
          IS(2) = KSTAR(I2)
...
***       GO TO 45
      ELSE
          ...
          IF ((KZ(27).EQ.1).OR.(KZ(27).EQ.2))
     &    write(6,*) 'KSTIDE #27=3 mode:', TTOT, TTOT*TSTAR,
     &                SEMI*RAU, ECC,
     &                KSTAR(I1), KSTAR(I2), NAME(I1), NAME(I2),
     &                BODY(I1)*SMU, BODY(I2)*SMU
      END IF
*
...
*       Determine pericentre variables U & UDOT by KS transformation.
      CALL KSPERI(IPAIR)
      H(IPAIR) = H(IPAIR) + DH
*
*       Print first and last energy change and update indicator.
      IF (KZ(27).LE.2.AND.(DUBCMP.EQV..FALSE.).AND.
     &   (KSTAR(I).EQ.0.OR.KSTAR(I).EQ.9.OR.KSTAR(I).EQ.-2)) THEN
          P = DAYS*SEMI1*SQRT(SEMI1/BODY(I))
          IF (KSTAR(I).EQ.0.AND.ECC1.GT.ECCM) THEN
              WHICH1 = 'NEW CIRC'
              KSTAR(I) = 9
              IF (KZ(27).EQ.2) KSTAR(I) = -2
...
***   IF(KZ(27).EQ.2.AND.(DUBCMP.EQV..FALSE.).AND.LIST(1,I1).EQ.0)THEN
***       LIST(1,I1) = 1
***       LIST(2,I1) = N
***       NP0 = 1
***   END IF
...
*       Restore the values in case not thread-safe (SJA 1/2013).
          JLIST(1) = I1
          JLIST(2) = I2
      END IF
*
...
*       Check for hierarchical configuration with eccentric inner binary.
      IF (KZ(27).EQ.2.AND.(DUBCMP.EQV..FALSE.)
     &    .AND.ECC.GT.0.95.AND.HI.LT.0.0) THEN
          NP1 = LIST(1,I1) + 1
          DO 30 L = 2,NP1
              J = LIST(L,I1)
              RIJ2 = 0.0
   ...
   25         CONTINUE
     ...
     &                                  RDOT**2/(A1*(BODY(I) + BODY(J)))
                  ...
                  SR = RP/RA
                  ICIRC = -1
                  CALL INDUCE(IPAIR,J,EMAX,EMIN,ICIRC,TC,ANGLE,TG,EDAV)
...
*       Set one unperturbed period for small apocentre perturbation (#27=1).
      ...
      IF (GA.LT.GMIN.AND.KZ(27).LE.2.AND.(DUBCMP.EQV..FALSE.)) THEN
...
*       Record diagnostics for new synchronous orbit and activate indicator.
      IF (ECC.GT.ECCM.AND.ECC1.LT.ECCM.AND.KZ(27).LE.2
     &    .AND.(DUBCMP.EQV..FALSE.)) THEN
          NSYNC = NSYNC + 1
...
*       Include warning in case of eccentricity increase (PT only).
      ...
      IF (KZ(27).EQ.1.AND.(DUBCMP.EQV..FALSE.)
     &    .AND.ECC2.GT.MAX(ECC,ECCM)+1.0D-04) THEN
...
*       Reduce radius by DR/R to delay dissipation for small energy loss.
      IF (KZ(27).EQ.1.AND.(DUBCMP.EQV..FALSE.)
     &    .AND.DE(1)+DE(2).LT.1.0D-07*ZMU*ABS(HI)) THEN
          J1 = I1
...
*       Combine the two stars inelastically in case of chaos disruption.
   45 IF (KZ(27).EQ.2.AND.(DUBCMP.EQV..FALSE.).AND.IDIS.GT.0) THEN

...
*       Include safety limit to enforce termination.
      ...
      END IF
*
*       Enforce binary removal for sufficient middle distance.
      RX = MAX(1.0/RINV(1),1.0/RINV(NN-1))
      IF (NN.EQ.4.AND.((1.0/RINV(2).GT.2.0*RX.AND.GPERT.GT.0.002).OR.
     &    (GPERT.GT.0.1.AND.1.0/RINV(2).GT.RX))) THEN
          R1 = 1.0/RINV(1)
          R2 = 1.0/RINV(2)
          R3 = 1.0/RINV(3)
          KCASE = -3
*       Repeat specification of indices in case set to zero.
   ...
   75     FORMAT (' FOUR-BODY TERM!    KC IE JE R1 R2 R3 ',3I4,1P,3E9.1)
      ELSE IF (NN.EQ.4.AND.((RX.GT.3.0*RMIN.AND.GPERT.GT.0.002).OR.
     &    (GPERT.GT.0.1.AND.RX.GT.3.0*RMIN))) THEN
*       Check for removal of first or last distant single. (SJA 12/2016)
      CALL HPSORT(NN-1,RINV,ISORT)
      WRITE (6,76)  (ISORT(K),K=1,NN-1), (INAME(K),K=1,NN)
   76 FORMAT (' ISORT INAM  ',3I4,2X,6I4)
*     DO 78 L = 1,NN
*     LX = 3*(L - 1)
*     WRITE (6,77)  L, NAMEC(L), (XCH(LX+K),K=1,3), (VCH(LX+K),K=1,3)
*  77 FORMAT (' L NMC XCH VCH ',I4,I6,3F9.4,2X,3F7.2)
*  78 CONTINUE
          IESC = 0
          IF (ISORT(1).EQ.1) THEN
              IESC = INAME(1)
...
*       Ensure positive radial velocity for single escaper (include ABSORB).
   ...
   80             FORMAT (' FOUR-BODY TERM!    KC IE JE RX RDOT ',
     &                                         3I4,1P,E10.2,0P,F7.2)
              ...
              END IF
          END IF
      ELSE IF (NN.EQ.3.AND.RX.GT.20.0*RMIN) THEN
          IF (NPERT.EQ.0) KCASE = -3
...
*       Include provisional check of three-body system using perturbation.
      IF (NN.EQ.3) THEN
          IF (RX.GT.30.0*RMIN.AND.IESC.GT.0) THEN
              RD = X4(1,IESC)*XDOT4(1,IESC) + X4(2,IESC)*XDOT4(2,IESC)
     &                                      + X4(3,IESC)*XDOT4(3,IESC)
              IF (GPERT.GT.0.0.AND.RD.GT.0.0) THEN
                  KCASE = 1
              ELSE
                  KCASE = 0
              END IF
...
*       Include algorithm for NN = 5 (JESC = 0 is easiest).
      IF (NN.EQ.5.AND.RX.GT.0.7*RSUM) THEN
          IF (ISORT(1).EQ.1) THEN
              IESC = INAME(1)
              JESC = INAME(2)
              KCASE = -3
              IF (1.0/RINV(2).LT.3.0*RMIN) JESC = 0
          ELSE IF (ISORT(1).EQ.4) THEN
              IESC = INAME(NN-1)
              JESC = INAME(NN)
              KCASE = -3
              IF (1.0/RINV(3).LT.3.0*RMIN) THEN
                  IESC = JESC
                  JESC = 0
              END IF
          END IF

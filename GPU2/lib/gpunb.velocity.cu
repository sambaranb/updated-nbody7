...
#define NIMAX (NTHREAD * NIBLOCK) // 4096 or 2048
...
struct Force{
	...
	__device__ void reduce_with(const int mask){
		acc.x += __shfl_xor_sync(0xffffffff, acc.x, mask);
		acc.y += __shfl_xor_sync(0xffffffff, acc.y, mask);
		acc.z += __shfl_xor_sync(0xffffffff, acc.z, mask);
		pot   += __shfl_xor_sync(0xffffffff, pot  , mask);
		jrk.x += __shfl_xor_sync(0xffffffff, jrk.x, mask);
		jrk.y += __shfl_xor_sync(0xffffffff, jrk.y, mask);
		jrk.z += __shfl_xor_sync(0xffffffff, jrk.z, mask);
		int ntmp = __shfl_xor_sync(0xffffffff, nnb, mask);
		if(nnb>=0 && ntmp>=0){
			nnb += ntmp;
		}else{
			nnb = -1;
...
__device__ void warp_reduce_int(int inp, int *out){
	inp += __shfl_xor_sync(0xffffffff, inp, 1);
	inp += __shfl_xor_sync(0xffffffff, inp, 2);
	inp += __shfl_xor_sync(0xffffffff, inp, 4);
	inp += __shfl_xor_sync(0xffffffff, inp, 8);
# if NXREDUCE==32
	inp += __shfl_xor_sync(0xffffffff, inp, 16);
# endif
	*out = inp;
}
__device__ void warp_reduce_float8(float4 inp1, float4 inp2, float *out){
	...
	tmp4L.x += __shfl_xor_sync(0xffffffff, tmp4R.x, 4);
	tmp4L.y += __shfl_xor_sync(0xffffffff, tmp4R.y, 4);
	tmp4L.z += __shfl_xor_sync(0xffffffff, tmp4R.z, 4);
	tmp4L.w += __shfl_xor_sync(0xffffffff, tmp4R.w, 4);
	float4 tmp4;
	tmp4.x = (2&tid) ? tmp4L.z : tmp4L.x;
	tmp4.y = (2&tid) ? tmp4L.w : tmp4L.y;
	tmp4.z = (2&tid) ? tmp4L.x : tmp4L.z;
	tmp4.w = (2&tid) ? tmp4L.y : tmp4L.w;
	tmp4.x += __shfl_xor_sync(0xffffffff, tmp4.z, 2);
	tmp4.y += __shfl_xor_sync(0xffffffff, tmp4.w, 2);
	float2 tmp2;
	tmp2.x = (1&tid) ? tmp4.y : tmp4.x;
	tmp2.y = (1&tid) ? tmp4.x : tmp4.y;
	tmp2.x += __shfl_xor_sync(0xffffffff, tmp2.y, 1);

	tmp2.x += __shfl_xor_sync(0xffffffff, tmp2.x, 8);
# if NXREDUCE==32
	tmp2.x += __shfl_xor_sync(0xffffffff, tmp2.x, 16);
# endif
	if(tid < 8){
		out[tid] = tmp2.x;
	}
...
#pragma unroll
	for(int ioff=1; ioff<NXREDUCE; ioff*=2){
		int iy = __shfl_up_sync(0xffffffff, ix, ioff);
		if(xid>=ioff) ix += iy;
	}
	int iz = __shfl_up_sync(0xffffffff, ix, 1);

...
*       Specify virial radius and mean mass (pc & M_sun).
          ...
          ZMBAR = ZMASS/(N+NBIN0)
*
          write (6,53)  mscale, lscale, vscale, lscale/vscale
   53     format (/,12X,'SCALE: Conversion factors: MSCALE = ',F12.3,
     &                  ' M_sun;  LSCALE = ',F7.3,' pc;  VSCALE = ',
     &                    F8.3,' km/sec;  TSCALE = ', F8.3,' Myr')
      end if
*
...
*
          write (6,741)  NBIN0
  741     format (/,12X,'SCALE: ', I8, ' Binaries restored')
      END IF
*
...
*       Perform energy check after scaling.
  ...
  745     format (/,12X,'SCALE: Scaled total energy: ',F12.6)

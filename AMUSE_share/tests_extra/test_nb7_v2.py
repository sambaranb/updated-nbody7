...
zmb = 8.0 | units.MSun
rb = 1.5 | units.parsec
converter = nbody_system.nbody_to_si(NN*zmb, rb)
...
for pat in ("CMPACT", "ESC", "COAL", "NBSTAT", "OUT3*", "fort*", "STOP", "run.out", "err.out"):
...
inst.parameters.ZMBAR = zmb
inst.parameters.RBAR = rb
...
inst.parameters.ZMET = 0.005
inst.parameters.QE = 1.0e-02
inst.parameters.bse_alpha1 = 0.7
inst.parameters.bse_nsflag = 4
inst.DEPLOT_AMUSE = 2.0
inst.set_kz(27,1)
inst.set_kz(28,3)
inst.set_kz(19,3)
inst.set_kz(12,1)
...
print("\nCommencing long evolution test...")
tnext = 0.0*t_unit
while tnext < 60.0 * t_unit:
    tnext = inst.get_time() + 3.0 * t_unit
    inst.evolve_model(tnext)
    print(f"Time: {tnext/t_unit} Nb time or {tnext.in_(units.Myr)}")
    print(f"First particle NAME KSTAR (after evolution): {inst.particles[0].name} {inst.particles[0].kstar}")
    print(f"Last particle NAME KSTAR (after evolution): {inst.particles[-1].name} {inst.particles[-1].kstar}")
